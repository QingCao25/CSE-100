...
// Incrementing Works - Decrementing Does not
//    assign B[0] = (LD & Din[0]) | (~LD & ~UD & (CE ^ Q[0])) | (~LD & ~CE & UD & ~Q[0]) | (~LD & ~CE & ~UD & Q[0]);
//    assign B[1] = (LD & Din[1]) | (~LD & ~UD & ((CE & Q[0]) ^ Q[1])) | (~LD & ~CE & UD & (~Q[0] ^ Q[1])) | (~LD & ~CE & ~UD & Q[1]);
//    assign B[2] = (LD & Din[2]) | (~LD & ~UD & ((CE & Q[0] & Q[1]) ^ Q[2])) |(~LD & ~CE & UD & (~Q[0] & ~Q[1] ^ Q[2])) | (~LD & ~CE & ~UD & Q[2]);
//    assign B[3] = (LD & Din[3]) | (~LD & ~UD & ((CE & Q[0] & Q[1] & Q[2] ^ Q[3]))) |  (~LD & ~CE & UD & (~Q[0] & ~Q[1] & ~Q[2] ^ Q[3])) | (~LD & ~CE & ~UD & Q[3]);
//    assign B[4] = (LD & Din[4]) | (~LD & ~UD & ((CE & Q[0] & Q[1] & Q[2] & Q[3]) ^ Q[4])) | (~LD & ~CE & UD & (~Q[0] & ~Q[1] & ~Q[2] & ~Q[3] ^ Q[4])) | (~LD & ~CE & ~UD & Q[4]);
